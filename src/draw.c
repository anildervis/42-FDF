...
void draw_line(t_dot a, t_dot b, t_fdf *data)
{
    ...
    step = 0;
    x_point = (a.x * data->zoom + data->window_width / 2 + data->translation_x);
    y_point = (a.y * data->zoom + data->window_heigth / 2 + data->translation_y);
    x_step = (b.x - a.x) * data->zoom ;
    y_step = (b.y - a.y) * data->zoom ;
